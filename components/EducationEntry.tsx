...
import { FlagBackdrop } from "@/components/flags";

export function EducationEntry({ entry }: { entry: Entry }) {
  return (
    <article className="group relative overflow-hidden rounded-2xl border border-ink/15 p-6">
      {entry.flagCountry && <FlagBackdrop country={entry.flagCountry} />}
      <div className="relative">
        <div className="flex items-start gap-4">
          <InstitutionBadge logoUrl={entry.logoUrl} alt={entry.institution} wide={entry.wideLogo} />
        ...
        {entry.paper && (
          <p className="mt-4 text-sm">
            <a href={entry.paper.url} download className="underline decoration-accent decoration-2">
